"""Execute one claimed run of the supervisor, step by step, so it can be resumed after a crash."""
import time
...
def execute_run(claimed: Claimed, *, store: RunStore, db: SupportDb, providers: dict,
                worker_id: str, lease_seconds: float,
                on_step: Callable[[dict], None] | None = None,
                record_delay: float = 0.0) -> str:
    run_id = claimed.run_id
    thread = store.get_thread(claimed.thread_id)
    handle = thread["student_id"]
    system = SUPERVISOR_SYSTEM.format(handle=handle)
    tools = SupervisorTools(db, providers, handle, on_step=on_step)
